...
- Contract & Integration Tests: List the contract tests and integration scenarios that will be created. For any
   contract changes include a migration plan or compatibility rationale.
- Vendored Builds (if applicable): If the feature proposes vendoring a build artifact, the plan MUST include a
   short justification, the intended vendor directory (e.g., `/build-binary/`), the exact build command or CI run URL,
   and how checksums and license compliance will be demonstrated.

If any gate is violated, document the complexity rationale and proposed mitigations in the Complexity Tracking section.
