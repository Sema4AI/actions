...
def delete_records(
    ...
    if not where_conditions:
        raise ActionError(
            "No record IDs or metadata filters provided for deletion"
        )
...
def get_metadata_columns(
    ...
    result = get_connection().query(
        sql, 
        params={"datasource_name": data_source.datasource_name}
    )
    ...
    return Response(result={
        ...
        "debug_info": {
            ...
            "raw_value": (
                str(metadata_columns_raw)[:100] 
                if metadata_columns_raw else None
            )
        }
    })
...
def insert_records(
    data_source: KnowledgeBaseDataSource,
    req: KnowledgeBaseInsertRequest
) -> Response[bool]:
    """
    ...
    available_columns = metadata_columns_result.result.get(
        "available_columns", []
    )
