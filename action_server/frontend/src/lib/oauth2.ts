...
const useAsBase = baseUrl || `${window.location.protocol}//${window.location.host}`;
...
export const createOAuthProviderSettingsFromUserSettings = (
  ...
  if (settings.authorizationEndpoint && settings.authorizationEndpoint !== DEFAULT) {
    ...
    ret.authorizationEndpoint = defaults.authorizationEndpoint;
  }

  // Check if we have all the info we need!
  if (ret.server === undefined) {
    throw new Error(`Error: "server" is not configured in the OAuth2 Settings for ${provider}.`);
  }
  if (!ret.server) {
    throw new Error(`Error: "server" is not empty in the OAuth2 Settings for ${provider}.`);
  }

  if (ret.tokenEndpoint === undefined) {
    throw new Error(
      `Error: "tokenEndpoint" is not configured in the OAuth2 Settings for ${provider}.`,
    );
  }
  if (!ret.tokenEndpoint) {
    throw new Error(`Error: "tokenEndpoint" is not empty in the OAuth2 Settings for ${provider}.`);
  }

  if (ret.authorizationEndpoint === undefined) {
    throw new Error(
      `Error: "authorizationEndpoint" is not configured in the OAuth2 Settings for ${provider}.`,
    );
  }
  if (!ret.authorizationEndpoint) {
    throw new Error(
      `Error: "authorizationEndpoint" is not empty in the OAuth2 Settings for ${provider}.`,
    );
  }
  return ret;
};
