...
/**
 * These array buffer conversion functions are from Cyrpto API official documentation examples
...
export const getAESKey = async (key?: Uint8Array | undefined): Promise<CryptoKey> => {
  ...
  if (!key) {
    ...
    key = base64ToUint8Array(keyBas64);
  }

  return await crypto.subtle.importKey('raw', key, algo, false, ['encrypt', 'decrypt']);
};
