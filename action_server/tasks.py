...
def build_oauth_config(ctx: Context):
    ...
    file_name = "sema4ai-oauth-config.yaml"
    yaml_file_path = CURDIR / file_name
    file_api_url = f"repos/Sema4AI/oauth-public-configs/contents/{file_name}"
    dest_path = (
        CURDIR / "src" / "sema4ai" / "action_server" / "_oauth_config.py"
    )
    
    run(ctx, "gh", "api", "-H \"Accept: application/vnd.github.raw\"", file_api_url, f"> {yaml_file_path}")
    
    assert yaml_file_path.exists(), f"Expected {yaml_file_path} to exist."
    
    with open(yaml_file_path) as yaml_file:
        file_contents = {"oauth_config.yaml": yaml_file.read()}
    
    with open(dest_path, "w", encoding='utf-8') as stream:
        print(f"Writing OAuth config to : {dest_path}")
        stream.write(
            f"""# coding: utf-8
# Note: autogenerated file.
