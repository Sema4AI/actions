...
def test_get_data_frame_parquet_fallback(agent_dummy_server):
    """Test get_data_frame with Parquet format and JSON fallback."""
    from sema4ai.actions.agent import get_data_frame
    ...
    with patch(
        ...
        result = get_data_frame("parquet_data")

        # Verify the request was made correctly
        assert agent_dummy_server.last_request is not None
        assert agent_dummy_server.last_request["dataframe_name"] == "parquet_data"
        assert agent_dummy_server.last_request["thread_id"] == "test-thread-parquet"

        # Verify the response (JSON response from dummy server)
        assert result.name == "parquet_data"
        assert result.description == "Data for parquet_data"
        assert result.columns == ["product", "sales"]
        assert len(result.rows) == 2
